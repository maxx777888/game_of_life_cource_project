...
int main()
{
    ...
    //Обязательная строка для создания случайных чисел
    srand(time(0));

    //Инициализация файла out.txt   
    //fstream file("zone.txt", std::ios::in | std::ios::out | std::ios::trunc);
    int width_bord; //Ширина игрального поля
    int height_bord;//Высона игрального поля
    ...
    showZone(play_zone, width_bord, height_bord, g, l_cells);//Первоначальный вывод игровой таблицы
